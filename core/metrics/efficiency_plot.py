...
def make_efficiency_figure(config: DictConfig,
                           real_column: pd.Series,
                           fake_column: pd.Series,
                           feature_column: pd.Series,
                           weight_column: np.array,
                           quantiles) -> Tuple[str, plt.Figure]:
    # quantiles = [float(q) for q in quantiles]
    bins = np.quantile(feature_column.to_numpy(), np.linspace(0., 1., config.metric.efficiency.bins + 1))
    ...
    def calculate_efficiencies_or_their_ratios(df):
        ...
        if total <= 0:
            result = pd.Series(
                [np.nan] * (len(quantiles) * (3 if config.metric.efficiency.make_ratio else 6))
            )
            if config.metric.efficiency.make_ratio:
                ...
                result.index = (
                    ...
                    sum(([f'eff_real_err_low_{q}', f'eff_fake_err_low_{q}'] for q in quantiles), []) +
                    sum(([f'eff_real_err_high_{q}', f'eff_fake_err_high_{q}'] for q in quantiles), [])
                )
            return result
        ...
        if config.metric.efficiency.make_ratio:
            efficiencies = ((passed + 0.5) / (total + 1.)).clip(0, 1)
        else:
        ...
        if config.metric.efficiency.make_ratio:
            ...
            mode = (
                (efficiencies + 1. / (2. * total))
                / (1. + 1. / total)
            )
            delta = (
                (
                    efficiencies * (1 - efficiencies) / total
                    + 1. / (4. * total**2)
                ).clip(lower=0)**0.5
                / (1. + 1. / total)
            )
            ...
            errors_high.index = sum(
                ([f'eff_real_err_high_{q}', f'eff_fake_err_high_{q}'] for q in quantiles), []
            )
    ...
    if config.metric.efficiency.make_ratio:
        ...
        major_ticks = np.array([-1., -0.5, 0., 0.5, 1., 9., 49., 99., 499., 999.])
    ...
    for q in quantiles:
        if config.metric.efficiency.make_ratio:
            ...
            plt.errorbar(
                ...
                y=efficiencies[f'eff_ratio_{q}'] - 1.,
                xerr=(
...
def make_figures(config: DictConfig,
